...
export default class LoginBase extends Component {
  componentWillMount() {
    ...
    console.log('[LoginBase.componentWillMount] currentUser=', currentUser);
    if (currentUser != null) {
      console.log(`[LoginBase.componentWillMount] user sedang online ${currentUser.email}`);
      Actions.homev2({ type: ActionConst.RESET });
    } else {
      console.log('[LoginBase.componentWillMount] user sedang tidak online');
    }
  }
  _authUser() {
